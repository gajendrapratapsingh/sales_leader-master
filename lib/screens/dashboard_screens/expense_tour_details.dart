...
class _ResetPinState extends State<TeamExpensePendingDetails>
  ...
  Future _leadData() async {
    var response = await http.post(
      Uri.parse(URL + "exlistbytouruserid"),
      body: {
    ...
    if (response.statusCode == 200) {
      ...
      if (data['expense_list'].length != 0) {
        if (data['expense_list'][0]['status'] != 'Approved') {
          setState(() {
            show = true;
          });
        }
                      ...
                      child: ButtonTheme(
                        ...
                        child: Container(
                          child: RaisedButton(
                            ...
                            onPressed: () async {
                              var response = await http.post(
                                Uri.parse(URL + "touruserwisestatuschanged"),
                                body: {
                                  "auth_key": "VrdoCRJjhZMVcl3PIsNdM",
                                  "id": _userId.toString(),
                                  "user_id": created_by_id,
              ...
              Padding(
                ...
                child: Row(
                    ...
                    children: [
                      Expanded(
                        child: Text(
                            "Tour ID: " + snapshot.data['tour_id'].toString(),
                            style: TextStyle(
                                color: Colors.black,
                    ...
                    children: [
                      Expanded(
                        child: Text(
                            "User DA Price: " +
                                "\u20B9 " +
                                snapshot.data['da_price'].toString(),
                            style: TextStyle(
                                color: Colors.black,
                                fontWeight: FontWeight.w800,
                                fontSize: 16.0)),
