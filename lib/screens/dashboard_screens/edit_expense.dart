...
class _ResetPinState extends State<EditExpense> {
  ...
  Future _leadData() async {
    var response = await http.post(
      Uri.parse(URL + "expensereimbursments_view"),
      body: {
    ...
    if (response.statusCode == 200) {
      ...
      fromDateController =
          TextEditingController(text: data['expense_details'][0]['from_date']);
      toDateController =
          TextEditingController(text: data['expense_details'][0]['to_date']);
      expenseDateController =
          TextEditingController(text: data['expense_details'][0]['ex_date']);
      if (data['expense_details'][0]['from_city'] != null) {
        WidgetsBinding.instance.addPostFrameCallback((_) => key
            .currentState
            .textField
            .controller
            .text = data['expense_details'][0]['from_city']);
        WidgetsBinding.instance.addPostFrameCallback((_) => key1.currentState
            .textField.controller.text = data['expense_details'][0]['to_city']);
      }
  ...
  Future _getCityCategories(type) async {
    ...
    print(
        jsonEncode({"auth_key": "VrdoCRJjhZMVcl3PIsNdM", "category_id": type}));
    if (response.statusCode == 200) {
      setState(() {
        _loading = false;
      });
      ...
      setState(() {
        ...
        //  if(selectedCity==""||selectedCity==null){
        if (selectedCity == "") selectedCity = _city[0].FOURTH_LEVEL_NAME;
        // }
      });
  ...
  Future _gettourCategories() async {
    var response = await http.post(
      Uri.parse(URL + "tourplanlists"),
      body: {"auth_key": "VrdoCRJjhZMVcl3PIsNdM", "id": _userId, "status": ""},
      headers: {
        "Accept": "application/json",
      },
    );
  ...
  void getUsers() async {
    try {
      final response = await http.post(
        Uri.parse(URL + "getcitymaster"),
        body: {"auth_key": "VrdoCRJjhZMVcl3PIsNdM", "search": ""},
        headers: {
          "Accept": "application/json",
        },
      );
      ...
      onTap: () async {
        if (_formKey.currentState.validate()) {
          ...
          if (selectedCity == "Conveyance") {
            ...
            var uri = Uri.parse(URL + 'edit_expense_reimbursments');
  ...
  Future _onAddImageClick(int index) async {
    setState(() {
      _imageFile = ImagePicker.platform.pickImage(
          source: ImageSource.camera,
          imageQuality: 1,
          maxHeight: 640,
          maxWidth: 480) as Future<File>;
  ...
  Future _onAddImageClick1(int index) async {
    setState(() {
      _imageFile = ImagePicker.platform.pickImage(
          ...
          maxHeight: 600,
          maxWidth: 800,
          imageQuality: 30) as Future<File>;
...
//    var dir = await path_provider.getTemporaryDirectory();
  ...
  Widget _typeWidgets(_initialValue1, _initialValue2, _initialValue3,
      _initialValue4, _initialValue5, _initialValue6, _initialValue7) {
          ...
          Container(
            ...
            child: Column(children: <Widget>[
              ...
              labelStyle: TextStyle(color: Colors.black),*/ /*
                  ...
                  Expanded(
                    child: searchTextField1 = AutoCompleteTextField<User>(
                      key: key1,
                      clearOnSubmit: false,
                      suggestions: users,
                      style: TextStyle(color: Colors.black, fontSize: 16.0),
            ...
            Container(
              ...
              child: Column(
                children: <Widget>[
                  ...
                  _typeWidgets(
                    snapshot.data['expense_details'][0]['hotel_name'],
                    snapshot.data['expense_details'][0]['city'],
                    snapshot.data['expense_details'][0]['no_of_night'],
                    snapshot.data['expense_details'][0]['from_city'],
                    snapshot.data['expense_details'][0]['to_city'],
                    snapshot.data['expense_details'][0]['from_date'],
                    snapshot.data['expense_details'][0]['to_date'],
                  ),
                  SizedBox(
                    height: 20,
                  ),
                  ...
                  _commentTextBox(
                      snapshot.data['expense_details'][0]['comments']),
                  SizedBox(
                    height: 20,
                  ),
                  Align(
                  ...
                  snapshot.data['upload_photo'].length != 0
                      ? GridView.count(
                          crossAxisCount: 3,
                          shrinkWrap: true,
                          primary: false,
                          ...
                          children: List.generate(
                              snapshot.data['upload_photo'].length, (index) {
                            return Container(
                              height: 400,
                              decoration: BoxDecoration(
                                image: DecorationImage(
                                  image: CachedNetworkImageProvider(
                                      snapshot.data['attachment_url'] +
                                          snapshot.data['upload_photo'][index]
                                              ['image']),
                                  fit: BoxFit.fill,
                                ),
                              ),
    ...
    return GestureDetector(
      ...
      child: Scaffold(
        ...
        body: ModalProgressHUD(
          ...
          child: Form(key: _formKey, child: leadDetails()),
...
class District {
  ...
  factory District.fromJson(Map<String, dynamic> json) {
    return new District(
        FIFTH_LEVEL_ID: json['id'].toString(), FIFTH_LEVEL_NAME: json['name']);
  }
}
