...
class _ResetPinState extends State<AddVisits> {
  ...
  Future _getContactCategories(type) async {
    var response = await http.post(
      Uri.parse(URL + "connecttypelists"),
      body: {
  ...
  Future _getAddressCategories(type) async {
    var response = await http.post(
      Uri.parse(URL + "typewiselocation"),
      body: {
        "auth_key": "VrdoCRJjhZMVcl3PIsNdM",
        "id": _userId.toString(),
        "type": type.toString(),
          ...
          if (type != "Select Visit Type") {
            if (type != "Lead") {
              if (type == "Other Visit") {
                ...
                var uri = Uri.parse(URL + 'addvisit');
                ...
                uploadRequest.fields['locationaddress'] =
