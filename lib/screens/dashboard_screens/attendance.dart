...
/*  final toDateController = TextEditingController();
  ...
  Future _leadData() async {
    var response = await http.post(
      da_type == "Per km"
          ? Uri.parse(URL + "localDAKm_wise_details")
          : Uri.parse(URL + "localDAfixed_wise_details"),
      body: {
        ...
        if (snapshot.hasData) {
          if (snapshot.data['local_DA_list'].length != 0) {
            return Column(children: <Widget>[
              da_type == "Per km"
                  ? header(snapshot.data['total_km_distance'].toString(),
                      snapshot.data['total_amount'].toString())
                  : header1(snapshot.data['total_hours'].toString(),
                      snapshot.data['total_amount'].toString()),
              SizedBox(
                height: 25.0,
              ),
                          ...
                          child: da_type == "Per km"
                              ? TimelineTile(
                                  ...
                                  endChild: _RightChild(
                                    ...
                                    title: snapshot.data['local_DA_list'][index]
                                            ['visit_type'] +
                                        "       Distance: " +
                                    ...
                                    subtitle: "Visit Date: " +
                                        snapshot.data['local_DA_list'][index]
                                            ['visit_date'],
                                    message: snapshot.data['local_DA_list']
                                        [index]['location'],
                                  ),
